...
from openviking.service.agent_task_manager import AgentTaskManager # Import AgentTaskManager
...
class OpenVikingService:
    ...
    def __init__(
        ...
        self._agent_task_manager: Optional[AgentTaskManager] = None # Add AgentTaskManager attribute
    ...
    @property
    def agent_tasks(self) -> AgentTaskManager:
        """Get AgentTaskManager instance."""
        if not self._agent_task_manager:
            raise NotInitializedError("AgentTaskManager")
        return self._agent_task_manager
    ...
    async def initialize(self) -> None:
        ...
        agent_worktrees_base_path = os.path.join(
            "D:\\AI4S\\MicroHySeeker\\MicroHySeeker", "agent_worktrees"
        )
        os.makedirs(agent_worktrees_base_path, exist_ok=True) # Ensure the directory exists
        self._agent_task_manager = AgentTaskManager(
            base_workdir=agent_worktrees_base_path,
            repo_root="D:\\AI4S\\MicroHySeeker\\MicroHySeeker", # Pass the repository root
            agent_scripts_dir="D:\\AI4S\\MicroHySeeker\\MicroHySeeker\\AutoHySeeker\\agent_scripts" # Pass agent scripts directory
        )
        logger.info(f"AgentTaskManager initialized with base_workdir: {agent_worktrees_base_path}")
