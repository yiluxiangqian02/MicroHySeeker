...
class EmbeddingModelConfig(BaseModel):
    ...
    def validate_config(self):
        ...
        if self.provider not in ["openai", "volcengine", "vikingdb", "jina", "sentence_transformers"]:
            raise ValueError(
                f"Invalid embedding provider: '{self.provider}'. Must be one of: 'openai', 'volcengine', 'vikingdb', 'jina', 'sentence_transformers'"
            )

        # sentence_transformers: no API key required
