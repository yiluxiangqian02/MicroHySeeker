...
class ProgramStepType(Enum):
    ...
    FLUSH = "flush"           # 冲洗（含仅排空选项）
    EChem = "echem"           # 电化学
    BLANK = "blank"           # 空白/等待
    EVACUATE = "evacuate"     # [已弃用] 排空功能已合并到FLUSH
```
...
| `evacuate_only` | bool | **新增** 仅排空溶液，不注水冲洗 |

**注意**: 当 `evacuate_only=True` 时，后端应只执行排空操作（使用 Outlet 泵抽取溶液），不执行注水冲洗。

#### EVACUATE (排空) - **已弃用**
> ⚠️ 此步骤类型已弃用，功能已合并到 FLUSH 步骤的 `evacuate_only` 选项。
> 旧数据加载时会自动转换为 FLUSH + evacuate_only=True
...
位置: `src/ui/main_window.py` - `ExperimentProcessWidget`

### 后端调用方法

后端在执行实验步骤时，应调用以下方法更新 UI 显示的液位：

| 方法 | 参数 | 说明 |
|-----|------|------|
| `set_tank_levels(tank1, tank2)` | tank1, tank2: float (0-1) | 直接设置液位比例 |
| `update_tank_volume(tank_id, volume_ml)` | tank_id: 1或2, volume_ml: float | 根据体积更新液位 |
| `reset_tanks()` | 无 | 重置所有池子为空 |
| `set_pump_states(inlet, transfer, outlet)` | bool×3 | 设置泵激活状态 |
| `set_single_pump_state(pump_type, active)` | pump_type: str, active: bool | 设置单个泵状态 |

### 调用时机

| 实验步骤 | 调用方法 | 说明 |
|---------|---------|------|
| 实验开始前 | `reset_tanks()` | 清空液位显示 |
| PREP_SOL 完成 | `update_tank_volume(1, volume_ml)` | 更新反应池液位 |
| TRANSFER 中 | `set_single_pump_state('transfer', True)` | 显示Transfer泵激活 |
| TRANSFER 完成 | `update_tank_volume(1, old-转移量)`, `update_tank_volume(2, old+转移量)` | 更新两池液位 |
| FLUSH (evacuate_only=True) | `update_tank_volume(2, 0)` | 电化学池清空 |
| EChem 开始 | 无需调用 | 液位保持 |

### 池子参数

```python
# ExperimentProcessWidget 内部参数
tank1_max_volume_ml = 5.0   # 反应池最大体积 (mL)
tank2_max_volume_ml = 3.0   # 电化学池最大体积 (mL)
```
...
- **底部**: "计算组合数" 按钮 + 组合数显示标签 + 保存按钮
- **字体**: 步骤列表 10pt，表格 9pt，适度放大以便阅读
- **步骤列合并**: 同一步骤的多个变量在步骤列合并显示（rowSpan），只显示一次

#### UI 控件规范
- **所有 QDoubleSpinBox**: 统一设置 `setSingleStep(0.01)`，按上下键每次变动 0.01
...
| 列 | 说明 | 是否可编辑 |
|----|------|-----------|
| 步骤 | 格式: `{序号:02d}:{类型}` 如 "01:配液"，同一步骤合并显示 | 只读 |
| 变量 | 参数名称，配液显示 `{溶液名}/{参数名}` | 只读 |
| 初值 | 当前值，保留2位小数 | 只读 |
| 终值 | 扫描终止值 (步长0.01) | 可编辑 (QDoubleSpinBox) |
| 间隔 | 扫描步长 (步长0.01) | 可编辑 (QDoubleSpinBox) |
...
所有 `QDoubleSpinBox` 控件统一设置：
- `setSingleStep(0.01)` - 按上下键每次变动 0.01

适用于以下对话框：
- `program_editor.py` - 所有电化学参数、持续时间、体积等
- `combo_exp_editor.py` - 终值和间隔输入
- `config_dialog.py` - 配液浓度、冲洗时长
- `prep_solution.py` - 总体积、目标浓度
- `calibrate_dialog.py` - 目标体积、实际体积
- `manual_dialog.py` - 注射泵参数、蠕动泵参数

### 9.2 主界面自动缩放

主界面的以下组件支持根据窗口大小自动缩放：
- **泵状态指示图** (`PumpDiagramWidget`): 根据可用空间计算泵的大小
- **实验过程图** (`ExperimentProcessWidget`): 使用缩放因子调整所有元素位置和大小

缩放算法：
```python
# 泵状态指示图
pump_w = max(30, min(w // 12 - 5, 60))  # 宽度范围 30-60
pump_h = max(20, min(h // 2 - 10, 40))  # 高度范围 20-40

# 实验过程图
scale = min(w / 600, h / 200)  # 相对于 600x200 参考尺寸
```
...
| 1.3 | 2026-02-01 | 添加UI全局规范：SpinBox步长0.01、主界面自动缩放；更新组合实验步骤变量列表 |
| 1.4 | 2026-02-04 | **重大更新**: 1) 删除EVACUATE步骤类型，合并到FLUSH的evacuate_only选项; 2) 添加液位控制接口; 3) 更新电化学变量(CV删除扫描段数); 4) 泵状态指示居中放大+标注溶液浓度 |
