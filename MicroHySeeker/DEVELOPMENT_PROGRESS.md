...
| **阶段3** | 配液功能  | ✅ 完成   | ✅ Mock + UI   | ✅ 位置模式校准   |
| **阶段4** | 冲洗功能  | ✅ 完成   | ✅ Mock + UI   | ⏳ 待用户验证     |
| **阶段5** | 实验引擎  | 🔄 待开发 | -              | -                 |
...
| PositionCalibrateDialog       | `src/dialogs/position_calibrate_dialog.py` | ✅   |
...
- ✅ SR_VFOC位置模式校准（多点线性回归）
- ✅ 泵校准对话框（10-100圈测试）
- ✅ 手动控制位置模式操作

**位置模式校准特性** (2026-02-04):

- ✅ 多点线性回归：10, 20, 30, ..., 100 圈测试
- ✅ 实时显示: k(斜率), b(截距), R²相关系数
- ✅ 单个泵选择校准，避免并发冲突
- ✅ 校准结果自动保存到配置
- ✅ 前端状态更新（RS485连接状态、程序关闭自动断开）

**前端对接**:

- `src/dialogs/config_dialog.py` - 配置对话框 ✅
- `src/dialogs/prep_solution.py` - 配液对话框 ✅
- `src/dialogs/position_calibrate_dialog.py` - 位置校准对话框 ✅
- `src/dialogs/manual_control.py` - 手动控制（位置模式）✅
...
| RS485Wrapper (冲洗接口)     | `src/services/rs485_wrapper.py`        | ✅   |
| FlusherDialog              | `src/dialogs/flusher_dialog.py`        | ✅   |

**验证**:

- `test_stage4_integration.py` 通过 ✅ (6个集成测试)
- **Mock模式UI测试**: ✅ 完成
  * 冲洗通道配置正常
  * 三泵循环控制正常
  * 单独排空/移液正常
...
- ✅ Flusher 冲洗器驱动（支持Mock和硬件模式）
- ✅ 三阶段冲洗循环：INLET → TRANSFER → OUTLET
- ✅ 阶段状态机和定时器控制
- ✅ 单独排空操作（evacuate）
- ✅ 单独移液操作（transfer）
- ✅ 多循环支持（1-99次）
- ✅ 实时进度跟踪（总进度 + 阶段进度）
- ✅ 回调支持：阶段变化、循环完成、全部完成、错误
- ✅ RS485Wrapper 冲洗接口集成
- ✅ FlusherDialog UI集成

**冲洗流程特性**:

- ✅ 三泵配置：Inlet(进水)、Transfer(移液)、Outlet(出水)
- ✅ 独立转速和持续时间设置
- ✅ 泵地址唯一性检查（不与配液泵冲突）
- ✅ 工作类型唯一性检查（每种类型只能配置一次）
- ✅ 线程安全的状态管理
- ✅ 错误处理和紧急停止

**前端对接**:

- `src/dialogs/flusher_dialog.py` - 冲洗对话框 ✅
- `src/dialogs/config_dialog.py` - 冲洗通道配置验证 ✅
- `src/models.py` - FlushChannel 数据模型 ✅
- `src/ui/main_window.py` - 主窗口工具栏集成 ✅
...
**问题1**: 单次实验点击报错
- **原因**: `program_editor.py` 中使用了错误的枚举值 `ProgramStepType.EChem`
- **修复**: 统一修改为正确的 `ProgramStepType.ECHEM`
- **影响**: 单次实验编辑器现在可以正常打开

**问题2**: 泵地址重复配置
- **原因**: 配液通道和冲洗通道可以使用相同的泵地址
- **修复**: 在 `config_dialog.py` 中添加地址验证逻辑
- **功能**: 
  - `_get_used_pump_addresses()` - 检查所有已用地址
  - 添加通道时会警告："泵地址 X 已被配置，请选择其他地址"

**问题3**: 冲洗工作类型重复配置
- **原因**: 可以配置多个相同工作类型（如两个Inlet）的冲洗通道
- **修复**: 添加工作类型唯一性检查
- **功能**:
  - `_get_used_flush_work_types()` - 检查已用工作类型
  - 添加通道时会警告："工作类型 'X' 已被配置，每种类型只能配置一次"
