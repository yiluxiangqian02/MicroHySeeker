...
MIN_RPM = 0                         # 最小转速 (RPM)
DEFAULT_ACCELERATION = 0x10         # 默认加速度
...
EXPECTED_RESPONSE_LENGTH = {
    CMD_READ_ENCODER: 8,         # 地址(1) + 命令(1) + 位置(4) + 校验(1) + 帧头(1) = 8
    CMD_READ_SPEED: 6,           # 地址(1) + 命令(1) + 速度(2) + 校验(1) + 帧头(1) = 6
    CMD_READ_VERSION: 8,         # 版本信息
    CMD_READ_ALL_SETTINGS: 38,   # 所有设置参数
    CMD_READ_ALL_STATUS: 31,     # 所有状态参数
    CMD_READ_RUN_STATUS: 5,      # 运行状态
    CMD_ENABLE: 5,               # 使能确认
    CMD_SPEED: 5,                # 速度设置确认
    CMD_POSITION: 5,             # 位置设置确认
}
