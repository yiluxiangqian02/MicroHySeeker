"""
组合实验编辑器 - 参照 C# ComboExpEditor
- 单一表格显示所有步骤的所有变量
- 每行：步骤序号+类型、变量名、初值、终值、间隔
- 紧凑字体，一次看全所有变量
"""
from PySide6.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout,
    QPushButton, QLabel, QGroupBox, QWidget, QLineEdit,
...
FONT_TABLE = QFont("Microsoft YaHei", 8)
...
class ComboExpEditorDialog(QDialog):
    ...
    def __init__(self, experiment: Experiment, config: SystemConfig = None, parent=None):
        ...
        self.setGeometry(100, 50, 950, 650)
    ...
    def _init_ui(self):
        ...
        info_label = QLabel("修改终值和间隔，系统自动生成参数组合")
        ...
        self.step_list.setFont(FONT_TABLE)
        self.step_list.setMaximumWidth(250)
        self.step_list.setStyleSheet("font-size: 8pt; line-height: 1.1;")
        ...
        self.param_table.setColumnWidth(0, 100)
        self.param_table.setColumnWidth(2, 70)
        self.param_table.setColumnWidth(3, 70)
        ...
        preview_frame.setStyleSheet("background: #f5f5f5; border-radius: 4px; padding: 5px;")
        ...
        self.preview_label.setStyleSheet("color: #1976D2;")
        preview_inner.addWidget(self.preview_label)
        
        calc_btn = QPushButton("计算")
        calc_btn.setFont(FONT_SMALL)
        calc_btn.setStyleSheet("padding: 3px 10px;")
        calc_btn.clicked.connect(self._calculate_combos)
        preview_inner.addWidget(calc_btn)
        ...
        save_btn.setStyleSheet("background-color: #2196F3; color: white; padding: 6px 15px;")
        save_btn.clicked.connect(self._on_save)
        bottom_layout.addWidget(save_btn)
        
        cancel_btn = QPushButton("取消")
        cancel_btn.setFont(FONT_SMALL)
        cancel_btn.setStyleSheet("padding: 6px 15px;")
    ...
    def _load_all_params(self):
        ...
        for row, param_data in enumerate(all_params):
            ...
            step_item.setBackground(QColor("#f0f0f0"))
    ...
    def _get_step_params(self, step: ProgStep) -> List[tuple]:
        """获取步骤的可变参数列表
        ...
        if step.step_type == ProgramStepType.BLANK:
            ...
            if self.config and self.config.dilution_channels:
                for ch in self.config.dilution_channels:
                    params.append((ch.solution_name, "浓度(M)", 0.01))
            # 总体积
            if step.prep_sol_params:
            ...
            if step.ec_settings:
                ...
                params.append(("E0(V)", ec.e0 or 0))
                
                if ec.technique == ECTechnique.CV:
                    params.append(("EH(V)", ec.eh or 0.8))
                    params.append(("EL(V)", ec.el or -0.2))
                    params.append(("EF(V)", ec.ef or 0))
                    params.append(("扫速(V/s)", ec.scan_rate or 0.05))
    ...
    def _calculate_combos(self):
        ...
        for param_data in self.param_rows:
            ...
            if interval > 0.001 and abs(end_val - init_val) > 0.001:
                ...
                total *= count
                changed_params.append(f"{param_data['param_name']}: {count}")
        
        if changed_params:
            detail = " × ".join([str(c.split(":")[1].strip()) for c in changed_params])
