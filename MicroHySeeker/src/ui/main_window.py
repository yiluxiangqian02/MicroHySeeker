...
class PumpDiagramWidget(QFrame):
    """泵状态指示 - 1行6个共2行布局，12个泵完整显示，水平垂直居中，标注溶液名称和原浓度"""
    ...
    def paintEvent(self, event):
        ...
        pump_w = 70  # 泵宽度
        pump_h = 40  # 泵高度
        info_height = 30  # 信息区高度（溶液名+浓度）
        spacing_x = 12  # 水平间隔
        spacing_y = 8   # 垂直间隔
        
        # 计算总尺寸
        total_w = 6 * pump_w + 5 * spacing_x
        total_h = 2 * (pump_h + info_height) + spacing_y
    ...
    def _draw_pump(self, painter: QPainter, x: int, y: int, w: int, h: int, pump_id: int):
        ...
        painter.setFont(QFont("Microsoft YaHei", font_size_id, QFont.Bold))
        ...
        for ch in self.config.dilution_channels:
            if ch.pump_address == pump_id:
                solution_name = ch.solution_name[:8]  # 限制长度
                stock_conc = ch.stock_concentration
                break
        
        # 在泵下方显示信息
        painter.setPen(Qt.black)
        painter.setFont(QFont("Microsoft YaHei", font_size_info))
        
        info_y = y + h + 2
        if solution_name:
            # 第一行：溶液名称
            painter.drawText(x, info_y, w, 13, Qt.AlignCenter, solution_name)
            # 第二行：原浓度
            painter.setFont(QFont("Microsoft YaHei", font_size_info - 1))
            painter.setPen(QColor("#666666"))
            painter.drawText(x, info_y + 13, w, 12, Qt.AlignCenter, f"{stock_conc:.2f}M")
        else:
            painter.drawText(x, info_y, w, 13, Qt.AlignCenter, f"泵{pump_id}")


class ExperimentProcessWidget(QFrame):
    """实验过程区域 - 绘制Inlet/Transfer/Outlet泵、反应池、电化学池，自适应窗口大小
    
    === 后端接口 ===
    液位更新: set_tank_levels(tank1_level, tank2_level) 或 update_tank_volume(tank_id, volume_ml)
    泵状态更新: set_pump_states(inlet, transfer, outlet) 或 set_single_pump_state(pump_type, active)
    后端应在以下时机调用:
      - 配液完成后: update_tank_volume(1, 新体积)  # 反应池
      - Transfer后: update_tank_volume(1, 旧-转移量), update_tank_volume(2, 旧+转移量)
      - 排空后: update_tank_volume(2, 0)  # 电化学池清空
      - 冲洗过程中: 根据实际情况更新
    """
    ...
    def __init__(self, config: SystemConfig, parent=None):
        ...
        self.tank1_level = 0.0  # 反应池液位 (0-1)，初始为空
        self.tank2_level = 0.0  # 电化学池液位 (0-1)，初始为空
        self.tank1_volume_ml = 0.0  # 反应池当前体积 (mL)
        self.tank2_volume_ml = 0.0  # 电化学池当前体积 (mL)
        self.tank1_max_volume_ml = 5.0  # 反应池最大体积 (mL)
        self.tank2_max_volume_ml = 3.0  # 电化学池最大体积 (mL)
    ...
    def set_single_pump_state(self, pump_type: str, active: bool):
        """设置单个泵的激活状态 - 供后端调用
        
        Args:
            pump_type: 'inlet', 'transfer', 或 'outlet'
            active: 是否激活
        """
        if pump_type.lower() == 'inlet':
            self.inlet_active = active
        elif pump_type.lower() == 'transfer':
            self.transfer_active = active
        elif pump_type.lower() == 'outlet':
            self.outlet_active = active
        self.update()
    ...
    def update_tank_volume(self, tank_id: int, volume_ml: float):
        """根据体积更新液位 - 供后端调用
        
        Args:
            tank_id: 1=反应池, 2=电化学池
            volume_ml: 当前体积 (mL)
        """
        if tank_id == 1:
            self.tank1_volume_ml = max(0, volume_ml)
            self.tank1_level = min(1.0, self.tank1_volume_ml / self.tank1_max_volume_ml)
        elif tank_id == 2:
            self.tank2_volume_ml = max(0, volume_ml)
            self.tank2_level = min(1.0, self.tank2_volume_ml / self.tank2_max_volume_ml)
        self.update()
    
    def reset_tanks(self):
        """重置所有池子为空 - 供后端在实验开始前调用"""
        self.tank1_level = 0.0
        self.tank2_level = 0.0
        self.tank1_volume_ml = 0.0
        self.tank2_volume_ml = 0.0
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # 自适应缩放计算
        scale_x = w / 600.0  # 基准宽度600
        scale_y = h / 200.0  # 基准高度200
        scale = min(scale_x, scale_y, 1.5)  # 限制最大放大1.5倍
        scale = max(scale, 0.6)  # 限制最小缩放0.6倍
        
        # 自适应尺寸
        pump_w = int(55 * scale)
        pump_h = int(35 * scale)
        tank_w = int(80 * scale)
        tank_h = int(100 * scale)
        font_size = max(7, int(9 * scale))
        font_size_title = max(8, int(11 * scale))
        
        # 居中偏移
        total_w = pump_w + tank_w + pump_w + tank_w + pump_w + int(60 * scale) + int(40 * scale)
        offset_x = max(10, (w - total_w) // 2)
        offset_y = max(25, (h - tank_h - 30) // 2)
        
        # 组合实验进程指示 (右上角)
        painter.setPen(Qt.black)
        painter.setFont(QFont("Microsoft YaHei", font_size_title, QFont.Bold))
        painter.drawText(w - 180, 5, 170, 20, Qt.AlignRight, f"组合进程: {self.combo_progress}")
        
        # === 绘制Inlet泵 ===
        inlet_x = offset_x
        inlet_y = offset_y + int(20 * scale)
        self._draw_pump_shape(painter, inlet_x, inlet_y, pump_w, pump_h, "Inlet", 
                              self.inlet_pump, self.inlet_active, scale)
        
        # === 绘制反应池 ===
        tank1_x = inlet_x + pump_w + int(15 * scale)
        tank1_y = offset_y
        self._draw_tank(painter, tank1_x, tank1_y, tank_w, tank_h, "反应池", 
                        self.tank1_level, QColor("#BBDEFB"), scale)
        
        # Inlet到反应池的管道
        pipe_color = QColor("#4CAF50") if self.inlet_active else QColor("#9E9E9E")
        pipe_width = max(2, int(3 * scale))
        painter.setPen(QPen(pipe_color, pipe_width))
        painter.drawLine(inlet_x + pump_w, inlet_y + pump_h // 2, tank1_x, tank1_y + int(25 * scale))
        
        # === 绘制Transfer泵 ===
        transfer_x = tank1_x + tank_w + int(15 * scale)
        transfer_y = offset_y + int(20 * scale)
        self._draw_pump_shape(painter, transfer_x, transfer_y, pump_w, pump_h, "Transfer",
                              self.transfer_pump, self.transfer_active, scale)
        
        # 反应池到Transfer泵的管道
        pipe_color = QColor("#2196F3") if self.transfer_active else QColor("#9E9E9E")
        painter.setPen(QPen(pipe_color, pipe_width))
        painter.drawLine(tank1_x + tank_w, tank1_y + tank_h // 2, transfer_x, transfer_y + pump_h // 2)
        
        # === 绘制电化学池 ===
        tank2_x = transfer_x + pump_w + int(15 * scale)
        tank2_y = offset_y
        self._draw_tank(painter, tank2_x, tank2_y, tank_w, tank_h, "电化学池",
                        self.tank2_level, QColor("#E1BEE7"), scale)
        
        # Transfer泵到电化学池的管道
        painter.setPen(QPen(pipe_color, pipe_width))
        painter.drawLine(transfer_x + pump_w, transfer_y + pump_h // 2, tank2_x, tank2_y + int(25 * scale))
        
        # === 绘制Outlet泵 ===
        outlet_x = tank2_x + tank_w + int(15 * scale)
        outlet_y = offset_y + int(20 * scale)
        self._draw_pump_shape(painter, outlet_x, outlet_y, pump_w, pump_h, "Outlet",
                              self.outlet_pump, self.outlet_active, scale)
        
        # 电化学池到Outlet泵的管道
        pipe_color = QColor("#FF9800") if self.outlet_active else QColor("#9E9E9E")
        painter.setPen(QPen(pipe_color, pipe_width))
        painter.drawLine(tank2_x + tank_w, tank2_y + tank_h // 2, outlet_x, outlet_y + pump_h // 2)
        
        # Outlet到废液的管道
        painter.drawLine(outlet_x + pump_w, outlet_y + pump_h // 2, outlet_x + pump_w + int(30 * scale), outlet_y + pump_h // 2)
        
        # 废液标识
        painter.setFont(QFont("Microsoft YaHei", font_size))
        painter.setPen(Qt.black)
        painter.drawText(outlet_x + pump_w + int(5 * scale), outlet_y + pump_h + int(15 * scale), "废液")
    
    def _draw_pump_shape(self, painter: QPainter, x: int, y: int, w: int, h: int,
                         name: str, pump_id: int, is_active: bool, scale: float = 1.0):
        """绘制泵的实际形状，自适应尺寸"""
        font_size = max(7, int(9 * scale))
        roller_r = max(4, int(6 * scale))
        ...
        painter.setPen(Qt.black)
        painter.setFont(QFont("Microsoft YaHei", font_size))
        painter.drawText(x, y - int(12 * scale), w, int(12 * scale), Qt.AlignCenter, name)
        if pump_id > 0:
            painter.drawText(x, y + h + 2, w, int(12 * scale), Qt.AlignCenter, f"泵{pump_id}")
    
    def _draw_tank(self, painter: QPainter, x: int, y: int, w: int, h: int,
                   name: str, level: float, liquid_color: QColor, scale: float = 1.0):
        """绘制烧杯/池子，带液位，自适应尺寸"""
        font_size = max(7, int(9 * scale))
        taper = int(10 * scale)  # 梯形收口
        
        # 容器轮廓 (梯形)
        path = QPainterPath()
        path.moveTo(x + taper, y)
        path.lineTo(x + w - taper, y)
        path.lineTo(x + w, y + h)
        path.lineTo(x, y + h)
        path.closeSubpath()
        
        painter.setPen(QPen(Qt.black, max(1, int(2 * scale))))
        ...
        if level > 0:
            ...
            # 计算液位处的宽度
            ratio = liquid_h / h
            left_offset = taper * (1 - ratio)
            right_offset = taper * (1 - ratio)
        ...
        painter.setPen(Qt.black)
        painter.setFont(QFont("Microsoft YaHei", font_size))
        painter.drawText(x, y + h + int(5 * scale), w, int(18 * scale), Qt.AlignCenter, name)
